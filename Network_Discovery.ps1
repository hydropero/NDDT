...
Foreach($item in $Output_IP_array)  {
     New-Object PSCustomObject -Property @{
        'IP Address'=$Output_IP_array[$count];
        'Mac Address'=$Output_MAC_array[$count];
        'Hostname'=$Output_Host_array[$count];
        'OUI Vendor'=$Output_OUI_array[$count]
        }
     $count = $count + 1     
}
$MyNetworkInfo.Hostname
}
